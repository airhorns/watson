(function() {
  var $extend, Data, Utils, cli, crypto, defaultStack, destroyRecordsForWorkerIds, printBrowser, printWorker,
    __slice = [].slice,
  ...
  destroyRecordsForWorkerIds = function(workerIds, inclusive, callback) {
    var condition, sql;

    if (inclusive == null) {
      inclusive = true;
    }
    sql = workerIds.length > 0 ? (condition = inclusive ? "" : "NOT", "worker_id " + condition + " IN (" + (workerIds.join()) + ")") : "1=1";
    return Data.BrowserWorker.findAll({
      where: sql
    }).fail(function(e) {
      throw e;
      ...
      for (_i = 0, _len = records.length; _i < _len; _i++) {
        record = records[_i];
        record.destroy().fail(function(e) {
          throw e;
        });
      }
  ...
  exports.command = function(args, options, config) {
    var client, url, urls, workerIds, _i, _len, _results;
    ...
    switch (args[0]) {
      case 'kill_all':
        return client.getWorkers(function(err, workers) {
          ...
          destroyRecordsForWorkerIds(workers.map(function(w) {
            return w.id;
          }), true);
        ...
        for (_i = 0, _len = urls.length; _i < _len; _i++) {
          ...
          _results.push(defaultStack.forEach(function(settings) {
            ...
            return client.createWorker(settings, function(err, worker) {
              ...
              return Data.BrowserWorker.create({
                token: token,
                worker_id: worker.id,
                note: settings.url
              }).fail(function(err) {
                ...
                return cli.info("Created worker ID=" + worker.id + " heading to " + settings.url + " using " + (printBrowser(settings)));
              });
            });
          }));
        }
      ...
      case 'kill_some':
        ...
        return workerIds.forEach(function(id) {
          return client.terminateWorker(id, function(err) {
            if (err) {
              throw err;
            }
            return cli.info("Terminated worker " + worker.id);
      ...
      case 'sync':
        return client.getWorkers(function(err, workers) {
          ...
          ids = workers.map(function(w) {
            return w.id;
          });
          return destroyRecordsForWorkerIds(ids, false, function(records) {
            cli.info("Destroyed " + records.length + " extraneous worker record(s)");
            return cli.info("Workers listed by the API: \n - " + workers.map(printWorker).join('\n - '));
          });
        });
      default:
