(function() {
  var TEMPLATE, Trackers, Watson, browserify, cli, coffee, coffeeify, express, fs, path, url;
  ...
  exports.command = function(args, options, config) {
    ...
    app.post('/save_results', function(req, res, next) {
      ...
      switch (data.metric) {
        case 'time':
          report = new Trackers.TimeTracker(data.name, function() {});
          return report._saveReport(data.bench, function(err) {
            if (err) {
              return next(err);
            } else {
